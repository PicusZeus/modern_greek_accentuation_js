        ...
        if (vowels.includes(letter.toLowerCase())) {
            ...
            if (all_letters.slice(border).length > 1) {
                if (diphtongs.includes(all_letters[border + 1] + letter) && true_syllabification) {
                    border += 1;
                } else if (diphtongs.includes(all_letters[border + 1] + letter) && !['αη', 'οη', 'άη', 'όη', 'άι', 'αϊ', 'όι', 'οϊ'].includes(all_letters[border + 1] + letter) && !true_syllabification) {
                    border += 1;
                }

                if (all_letters.slice(border + 1).length > 0) {
                    if (true_syllabification) {
                        if (all_letters.slice(border + 1).length > 1 && un_digraph_i.includes(all_letters[border + 2] + all_letters[border + 1])) {
                            border += 2;
                        } else if (un_single_i.includes(all_letters[border + 1]) && all_letters.slice(border + 1).length > 1 && !['ευ', 'εύ', 'αυ', 'αύ'].includes(all_letters[border + 2] + all_letters[border + 1])) {
                            border += 1;
                        }
                    }
